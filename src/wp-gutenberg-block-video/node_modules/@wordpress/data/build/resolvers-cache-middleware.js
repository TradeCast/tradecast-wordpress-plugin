...
const createResolversCacheMiddleware = (registry, reducerKey) => () => next => action => {
  const resolvers = registry.select(_name.STORE_NAME).getCachedResolvers(reducerKey);
  Object.entries(resolvers).forEach(_ref => {
    ...
    resolversByArgs.forEach((value, args) => {
      ...
      registry.dispatch(_name.STORE_NAME).invalidateResolution(reducerKey, selectorName, args);
    });
  });
  return next(action);
};
