<template>
  <div class="tradecast-add-popular-gallery-content-container">
    <tradecast-gallery-header
      :title="t('modals.galleries.popular.title')"
      :content="t('modals.galleries.popular.content')"
    />
...
export default defineComponent({
  name: 'tradecast-add-popular-gallery-content',
  ...
  setup(props, { emit }) {
    ...
    const data = reactive({
      ...
      videosLoading: false,
      gallery: { title: t('modals.galleries.popular.name'), type: GalleryTypeEnum.POPULAR } as GalleryContentType,
      page: 1,
    ...
    /**
     * Loads popular videos.
     */
    const loadVideos = (): void => {
      data.videosLoading = true;
      actions
        .getMediaList({ sort: { amountViewsMonth: 'desc' }, page: data.page, limit: data.limit })
