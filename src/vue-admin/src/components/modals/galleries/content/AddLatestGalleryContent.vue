<template>
  <div class="tradecast-add-latest-gallery-content-container">
    <tradecast-gallery-header
      :title="t('modals.galleries.latest.title')"
      :content="t('modals.galleries.latest.content')"
    />
...
export default defineComponent({
  name: 'tradecast-add-latest-gallery-content',
  ...
  setup(props, { emit }) {
    ...
    const data = reactive({
      ...
      videosLoading: false,
      gallery: { title: t('modals.galleries.latest.name'), type: GalleryTypeEnum.LATEST } as GalleryContentType,
      page: 1,
    ...
    const loadVideos = (): void => {
      ...
      actions
        .getMediaList({ sort: { publishStart: 'desc' }, page: data.page, limit: data.limit })
